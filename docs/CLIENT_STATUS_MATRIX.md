...
| Codex CLI local-file workflow | `configured` | [Codex CLI Compatibility Notes](CODEX_CLI_COMPATIBILITY_NOTES.md), [Agent Prompt Packs](AGENT_PROMPT_PACKS.md), and [local-file recipe](PLATFORM_AGENT_SMOKE_RECIPES.md#claude-code-or-codex-cli-local-file-smoke-test) | Test from a real Codex CLI environment and report in [issue #14](https://github.com/sunnja69/akephalos/issues/14). |
...
| Cline / Roo Code MCP clients | `configured` | [MCP Client Config Snippets](MCP_CLIENT_SNIPPETS.md#cursor--vs-code-mcp-style), [VS Code MCP Client Notes](VS_CODE_MCP_CLIENT_NOTES.md), and [Cline/Roo MCP Client Notes](CLINE_ROO_MCP_CLIENT_NOTES.md) | Test one VS Code client and report in [issue #16](https://github.com/sunnja69/akephalos/issues/16). |
